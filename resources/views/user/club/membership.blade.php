                                ...
                                <div class="row">
                                    <div class="col-md-12 mt-4">
                                        <ul class="user-plan-table">
                                            <li>
                                                <h4>{{__('Membership Status  ')}} <span>{{ !empty($club->plan_id) ? isset($club->plan->plan_name) ? $club->plan->plan_name : ''. __(' Member') : __('Normal Member')}}</span></h4>
                                            </li>
                                            <li>
                                            ...
                                            <li>
                                                <h4>{{__('Earned Bonus ')}} <span>{{ user_plan_bonus($club->user_id) }}</span></h4>
                                            </li>
                                        </ul>
                                    </div>
                                </div>
                                ...
                                <div class="row">
                                    @if(!empty($club->plan_id))
                                        <div class="col-md-12 mt-4">
                                            <ul class="user-plan-table">
                                                <li class="user-t-img">
                                                    @if(isset($club->plan->image))
                                                    <img src="{{show_plan_image($club->plan_id,$club->plan->image)}}" class="img-fluid cp-user-logo-large" alt=""> @endif
                                                </li>
                                                <li>
                                                    <h4>{{__('Plan Name  ')}} <span>{{isset($club->plan->plan_name) ? $club->plan->plan_name : '' }}</span></h4>
                                                </li>
                                                <li>
                                                    <h4>{{__('Blocked Amount  ')}} <span>{{number_format($club->amount,2)}} {{settings('coin_name')}}</span> </h4>
                                                </li>
