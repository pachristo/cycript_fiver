        ...
        try {
            ...
            if (isset($members[0])) {
                ...
                foreach ($members as $member) {
                    ...
                    if (isset($find_plan)) {
                        $wallet = get_primary_wallet($member->user_id, $find_plan->bonus_coin_type);
                        ...
                        if (empty($bonus_distribution)) {
                            $wallet->increment('balance',$btcBonusAmmount);
                            $member->update([
                                'start_date'=>date('Y-m-d'),
                                'end_date'=>date('Y-m-d', strtotime(date('Y-m-d'). ' + '.$find_plan->duration.' days'))
                            ]);
                            $save = $this->saveBonusDistributionHistory($member->user_id,$wallet->id,$member->plan_id,$member->id,date('Y-m-d'),$btcBonusAmmount,$find_plan->bonus,$find_plan->bonus_type,$find_plan->bonus_coin_type,$bonus);
                        }
                    }
                }
